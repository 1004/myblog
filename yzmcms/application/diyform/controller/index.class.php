...
class index{
	...
	public function show(){
		...
		$data = $this->modelinfo;
		$title =  $data['name'];
		$template = $data['show_template'];
		
		//SEO相关设置
		$site = get_config();
		$seo_title = $title.'_'.$site['site_name'];
		$keywords = $site['site_keyword'];
		$description = $data['description'] ? $data['description'] : $site['site_description'];
		
		//获取当前位置
		$location = '<a href="'.SITE_URL.'">首页</a> &gt; <a href="'.U('init').'">'.$data['name'].'</a> &gt;显示页';
